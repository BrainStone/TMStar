...
int main() {
	...
	buffer.buffer.writeObj(Message<ProcessedCallData>{MessageType::C_PROCESSED_CALL, ErrorCode::NONE,
	                                                  ProcessedCallData{MessageType::S_ON_REGISTERED}});
	buffer.buffer.buffer[1] = 0xFF;
	...
	Message<ExecuteCommandData> message{MessageType::C_EXECUTE_COMMAND, ErrorCode::NONE,
	                                    ExecuteCommandData{std::string{"help"}}};
